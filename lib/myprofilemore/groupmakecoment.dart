...
void main()=>runApp(GroupMakeComent());

class GroupMakeComent extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: "AWE",
      debugShowCheckedModeBanner: false,
      home: groupmakecomment(),

    );
  }
}

class groupmakecomment extends StatefulWidget {
  ...
  const groupmakecomment({Key key, this.newsfeed_image, this.newsfeed_name, this.newsfeed_message, this.newsfeed_like, this.newsfeed_id, this.page_from}) : super(key: key);




  _groupmakecommentState createState() => _groupmakecommentState();
}

class _groupmakecommentState extends State<groupmakecomment> {
  ...
  Future<List> _sendcomment() async{
    ...
    final response = await http.post(URL_CONNECTOR+"sendgroupmessage.php", body: {
      "message":message.text,
      "myaidno":userlogedinid,
      "groupid":widget.newsfeed_id.toString(),
    });
      ...
      if(datauser[0]['code']=="Fail"){
        ...
        if(widget.page_from=="myothergroup"){
          Navigator.pushReplacement(context,
              MaterialPageRoute(builder: (BuildContext ctx) => myothergroups()));
        }else{
          Navigator.pushReplacement(context,
              MaterialPageRoute(builder: (BuildContext ctx) => mygroups()));
        }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: GestureDetector(
          onTap: (){
            Navigator.push(context,
              MaterialPageRoute(builder: (context) => groupexpand2()),);
          },
          child: IconButton(
            icon: Icon(
